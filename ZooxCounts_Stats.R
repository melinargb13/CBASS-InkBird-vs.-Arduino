...
Full_Physiology <-read.table("zoox_counts_melina_noout.txt", sep = "\t",header = TRUE)
...
print(emmeans(Sty_sym_final, list(pairwise ~ temp|system)), adjust = c("Bonferroni"))
print(emmeans(Sty_sym_final, list(pairwise ~ system|temp)), adjust = c("Bonferroni"))
...
#######################################################################
#######################################################################
#Zoox counts box plot (we wont use this one for visualization purposes#
#instead we use the bar plot)##########################################
#######################################################################
#######################################################################
...
Sty_sym <- ggplot(data=Sty_phys, 
  ...
  theme(line= element_line(size = 1),
        ...
        panel.spacing = unit(4, "lines")) + xlab(label = "temperature (°C)") + ylab(label = "Symbiont density (cm^2)")+
  theme(axis.text.x = element_text(color = "black", size = 12, angle = 0, hjust = .5, vjust = .5, face = "plain"),
        ...
        axis.title.y = element_text(color = "black", size = 14, angle = 90, hjust = .5, vjust = 2.5, face = "plain"),
