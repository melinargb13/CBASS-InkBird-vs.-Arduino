...
x<- ggplot() +
  geom_line( data = curve_T1_input, aes(x=Temperature, y=`Fv/Fm`, group=Sample, color=Treatment), size = 1) +
  geom_ribbon(data = curve_T1_input_preddata_1, aes(x = temp, ymin=Lower, ymax=Upper), color = 'darkgoldenrod1', linetype=2, alpha = 0.1, size=1) +
  geom_ribbon(data = curve_T1_input_preddata_2, aes(x = temp, ymin=Lower, ymax=Upper), color = 'royalblue2', linetype=2, alpha = 0.1, outline.type = "both", size=1) +
  geom_segment(data = curve_T1_input, aes(x =meanED50, y = 0, xend = meanED50, yend = 0.65,color=Treatment), linetype=3, size=1.5) +
  #geom_smooth (data = curve_T1_input, aes(x=Temperature, y=`Fv/Fm`, group=Sample, color=Treatment), method=glm , se =F) +
  geom_text(data = curve_T1_input, mapping=aes(x=meanED50, y=0.68, label=meanED50, color=Treatment), size=7, angle=90, hjust=0, check_overlap = T, show_guide = F) +
  ...
  geom_jitter(data = input_t1, aes(x = Temperature, y = PAM, color = Treatment), size = 3, width = 0.60) 
